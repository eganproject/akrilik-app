...
@section('content')
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-400">Perbarui data kategori.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.product-categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')
        @include('admin.product_categories._form', ['category' => $category])
    </form>
@endsection
